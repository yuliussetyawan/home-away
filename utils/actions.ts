...
export const fetchRentals = async () => {
  ...
  const rentalsWithBookingSums = await Promise.all(
    rentals.map(async (rental) => {
      // db.booking.aggregate(...) is used to perform aggregate calculations (like sum, count, average, min, max)
      ...
      const orderTotalSum = await db.booking.aggregate({
        ...
        // The underscore (_) is just a naming convention to clearly distinguish aggregate operations from normal fields in Prisma
        _sum: {
          orderTotal: true,
        },
      });
